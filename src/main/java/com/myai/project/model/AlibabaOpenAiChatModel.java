package com.myai.project.model;
...
public class AlibabaOpenAiChatModel extends AbstractToolCallSupport implements ChatModel {
    ...
    private MultiValueMap<String, String> getAdditionalHttpHeaders(Prompt prompt) {
        ...
        return CollectionUtils.toMultiValueMap(
                headers.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, e -> java.util.Collections.singletonList(e.getValue()))));
    }
    ...
    Prompt buildRequestPrompt(Prompt prompt) {
        ...
        if (prompt.getOptions() != null) {
            ...
            else if (prompt.getOptions() instanceof FunctionCallingOptions) {
                FunctionCallingOptions functionCallingOptions = (FunctionCallingOptions) prompt.getOptions();
            ...
            if (message.getMessageType() == MessageType.USER || message.getMessageType() == MessageType.SYSTEM) {
                ...
                if (message instanceof UserMessage) {
                    ...
                    if (!CollectionUtils.isEmpty(userMessage.getMedia())) {
                        List<OpenAiApi.ChatCompletionMessage.MediaContent> contentList = new ArrayList<>(java.util.Collections.singletonList(new OpenAiApi.ChatCompletionMessage.MediaContent(message.getText())));

                        contentList.addAll(userMessage.getMedia().stream().map(this::mapToMediaContent).collect(java.util.stream.Collectors.toList()));
